...
/**
 * A utility class for IP seeking.
 * 
 */
public class IPAddress implements InitializingBean {
	
	protected static final Log LOG = LogFactory.getLog(IPAddress.class);
	
	/** Whether the {@link #IPAddress} had already been initialized */
	...
	/** The IP data path */
	public static final String IPDATA_FILE_PATH_DEFALT = "conf/qqwry.dat";
	private static final String IPDATA_FILE_KEY = "ip.data.file";
	 ...
	 * @return 国家名字符串
	 */
	protected String getCountry(byte[] ip) {
		// 检查ip地址文件是否正常
		if (ipFile == null) {
			// return LumaQQ.getString("bad.ip.file");
			return "bad.ip.file";
	 ...
	 * @return 地区名字符串
	 */
	protected String getArea(byte[] ip) {
		// 检查ip地址文件是否正常
		if (ipFile == null) {
			// return LumaQQ.getString("bad.ip.file");
			return "bad.ip.file";
	...
	protected void init() throws Exception {
		...
		b4 = new byte[4];
		b3 = new byte[3];
		Resource cpr = new ClassPathResource(IPDATA_FILE_PATH);
		ipFile = new RandomAccessFile(cpr.getFile(), "r");
		if (ipFile == null) {
			throw new IllegalStateException("Failed to get the ip data file.");
		}
		// 如果打开文件成功，读取文件头信息
		ipBegin = readLong4(0);
		ipEnd = readLong4(4);
		if (ipBegin == -1 || ipEnd == -1) {
			ipFile.close();
			ipFile = null;
			LOG.error("The ip data file format error.");
		}
	}
