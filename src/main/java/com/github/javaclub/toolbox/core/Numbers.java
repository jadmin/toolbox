...
public abstract class Numbers {
	...
	public static int random(int max) {
		Assert.isTrue(max > 0, "the parameter [max]'s value must be a positive Integer.");
	...
	public static double random(double max) {
		Assert.isTrue(max > 0, "the parameter [max]'s value must be a positive Double.");
    ...
    public static Double[] toDoubleArray(double[] array) {
    	...
    	Double[] iarray = new Double[array.length];
    	
    	for (int i = 0; i < array.length; i++) {
			iarray[i] = new Double(array[i]);
		}
    	return iarray;
    }
    
    public static Float[] toDoubleArray(float[] array) {
    	if(null == array) {
    		return null;
    	}
    	Float[] iarray = new Float[array.length];
    	
    	for (int i = 0; i < array.length; i++) {
			iarray[i] = new Float(array[i]);
		}
    	return iarray;
    }
    
    public static String format(int val, int len, boolean left) {
