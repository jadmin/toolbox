...
public class FileUtil {
	...
	/**
	 * Copy deep tree file from source to target that satisfy the specified
	 * filter
	 * 
	 * @param source
	 * @param target
	 ...
	 * @throws IOException
	 */
	public static void copyTree(File source, File target, FileFilter fileFilter)
			throws IOException {
		Assert.isTrue(source != null && (source.isDirectory()),
				"Source File must denote a directory.");
		Assert.isTrue(target != null && (target.isDirectory()),
				"Destinate File must denote a directory.");
		_copyTree(source, target, fileFilter);
	}

	private static void _copyTree(File sourceDir, File targetDir, FileFilter fileFilter) throws IOException {
		File[] rootFiles = sourceDir.listFiles();
		File[] rootFilterdFiles = sourceDir.listFiles(fileFilter);
		for (File file : rootFiles) {
			if(ObjectUtil.contains(rootFilterdFiles, file)) {
				if(file.isFile()) {
					copy(file, targetDir);
				} else if(file.isDirectory()) {
					// 先将目录创建好
					File dir = createDir(new File(targetDir, file.getName()));
					_copyTree(file, dir, fileFilter);
				} else {
					// do nothing
				}
			} else {
	...
	/**
	 * Using this method to generate a uniqueness filename
	 *
